...
const updatesalemanagemnet = () => {
  ...
  useEffect(() => {
    const fetchPumpSheetData = async () => {
      try {
        const response = await axios.get(`http://localhost:5500/mastersheet/finance/${id}`)
  ...
  const handleDelete = async (e) => {
    ...
    try {
      if (window.confirm("Are you sure you want to delete this purchase management sheet?")) {
        const response = await axios.delete(`http://localhost:5500/mastersheet/finance/${id}`)
  ...
  const handleSave = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5500/mastersheet/finance/${id}`, data)
