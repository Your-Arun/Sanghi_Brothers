...
import {
  Menu, Users, FileText, Share2, Calendar, Plus, RefreshCw, X, Download, Wind, AlertCircle, LayoutDashboard, Trash2
} from 'lucide-react';
import {
  DndContext,
  DragOverlay,
  ...
  return (
    ...
    >
      <div className={`${imgSize} rounded-full overflow-hidden border-4 border-white shadow-md bg-gray-200 transition-all`}>
        <img
          src={staffMember.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(staffMember.name)}&background=random`}
...
const DroppableZone = ({ id, children, className = '', label, isAbsent = false, isAir = false, isPool = false }) => {
  ...
  if (isOver) {
    ...
    else if (isPool) activeClass = 'bg-blue-100 border-blue-500 scale-105 shadow-xl';
...
const ShiftManagementSystem = () => {
  ...
  const [assignments, setAssignments] = useState({}); // keys: N1..N6, Air, absent (array)
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        ...
        const formattedMembers = response.data.map(m => ({ ...m, id: m._id, avatar: m.avatar || null }));
        setMembers(formattedMembers);
        const absents = formattedMembers.filter(m => (m.available === 'absent' || m.available === 'leave'));
  ...
  const handleDragStart = (event) => {
    ...
    const staff = members.find((s) => s.id === active.id) || Object.values(assignments).flat().find(a => a?.id === active.id);
  ...
  const handleDragEnd = (event) => {
    ...
    // find staff either from members (fresh) or from assignments (if already assigned)
    const draggedStaff = members.find((s) => s.id === staffId) || Object.values(assignments).flat().find(a => a?.id === staffId);
    ...
    setAssignments((prev) => {
      ...
      // Ensure absent is array
      if (!Array.isArray(newAssignments['absent'])) newAssignments['absent'] = [];

      // Remove this staff from absent if present
      newAssignments['absent'] = newAssignments['absent'].filter((s) => s.id !== staffId);

      // Remove this staff from any single-slot assignment
      Object.keys(newAssignments).forEach((key) => {
        if (key !== 'absent' && newAssignments[key]?.id === staffId) newAssignments[key] = null;
      });

      // Place depending on target
      if (targetZone === 'absent') {
        // push into absent array (avoid duplicates)
        if (!newAssignments['absent'].find(s => s.id === staffId)) newAssignments['absent'] = [...newAssignments['absent'], draggedStaff];
      } else if (targetZone === 'available-pool' || targetZone === 'available') {
        // dropping back to pool -> nothing to do, already removed from assignments
      } else {
        // single-slot zones
  ...
  const handleAutoAssign = () => {
    const alreadyAssigned = Object.values(assignments).flat().map((s) => s?.id).filter(Boolean);
    const availableForShift = members.filter((m) => m.available === "present" && !alreadyAssigned.includes(m.id));
    ...
    let operators = availableForShift.filter(m => (m.role === 'operator' || m.role === 'pump operator'));
  ...
  const handleDeleteMember = async (id) => {
    if (window.confirm("Delete?")) {
      try {
        ...
        setAssignments(prev => {
          const copy = { ...prev };
          if (Array.isArray(copy.absent)) copy.absent = copy.absent.filter(a => a.id !== id);
          Object.keys(copy).forEach(k => {
            if (k !== 'absent' && copy[k]?.id === id) copy[k] = null;
          });
          return copy;
        });
                ...
                <tbody className="divide-y">
                  {members.map((m) => (
                    <tr key={m.id} className="hover:bg-blue-50">
                      <td className="p-3 font-bold flex items-center gap-2"><img src={m.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(m.name)}`} className="w-8 h-8 rounded-full" alt="" />{m.name}</td>
