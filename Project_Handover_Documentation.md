...
* **Registration Invitation Codes:** Users can only register if they provide a valid invitation code configured in the environment:
  - **Member/Manager/Backoffice Codes:** Configured via the `VALID_INVITATION_CODES` environment variable.
  - **Field Workers/Staff Codes:** Configured via the `VALID_INVITATION_CODES_FOR_STAFF` environment variable.
